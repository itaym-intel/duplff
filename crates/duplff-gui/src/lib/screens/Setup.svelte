...
<div class="flex items-center justify-center min-h-screen p-8">
  <div class="w-full max-w-lg">
    ...
    <div class="mb-10">
      ...
      <p class="text-sm text-text-muted ml-11">Find and remove duplicate files safely</p>
    </div>
      ...
      <div class="pt-2">
        <button
          ...
          class="w-full py-3 rounded-xl text-sm font-medium transition-all duration-200
            {roots.length > 0
              ? 'bg-active text-white shadow-lg shadow-active/20 hover:shadow-active/30 hover:brightness-110 active:scale-[0.99]'
              : 'bg-surface text-text-muted border border-border cursor-not-allowed'}"
        >
          {roots.length === 0 ? 'Select a folder to scan' : 'Scan for duplicates'}
        </button>
      </div>
    </div>
  </div>
